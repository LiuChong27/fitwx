			...
			return {
				servicesList: [{
					...
					},
					{
						...
						"path": "/pages/login/login-withoutpwd?type=huawei"
					},
					{
						"id": "huaweiMobile",
						"text": "华为账号一键登录",
						"logo": "/uni_modules/uni-id-pages/static/login/uni-fab-login/huawei.png",
						"path": "/pages/login/login-withoutpwd?type=huaweiMobile"
					},
					// #ifndef MP-WEIXIN
					{
						"id": "apple",
		...
		methods: {
			...
			toPage(path,index = 0) {
				...
				if (this.getRoute(1) == path.split('?')[0] && this.getRoute(1) ==
					...
					//如果要被打开的页面已经打开，且这个页面是 /pages/login/login-withoutpwd 则把类型参数传给他
